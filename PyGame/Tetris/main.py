...
def valid_space(shape, grid):
    ...
    fornatted = convert_shape_format(shape)

    for pos in fornatted:
...
def draw_grid(win,grid):
    for row in range(len(grid)+1):
        pygame.draw.line(win,(120,120,120),(top_left_x,top_left_y+row*block_size),(top_left_x+play_width,top_left_y+row*block_size))
    for col in range(len(grid[0])+1):
        pygame.draw.line(win, (120, 120, 120), (top_left_x + col*block_size, top_left_y),
...
def draw_window(win,grid,max_score,score=0):
    win.fill((0, 0, 0))
    ...
    # max score
    label = font.render('High Score: ' + str(max_score), 1, (255, 255, 255))
    sx = top_left_x - 200
    sy = top_left_y + 200
    win.blit(label, (sx-30, sy + 160))
...
def main(win):
    ...
    while run:
        ...
        # every 5 seconds the speed at which the piece falls increases
        ...
        draw_window(win,grid,max_score,score)
