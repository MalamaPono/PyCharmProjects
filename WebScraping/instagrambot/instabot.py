...
class InstagramBot:
    ...
    def UnfollowUser(self, user):
        ...
        self.GoToUser(user)
        div = self.driver.find_element_by_xpath("//*[@id='react-root']/div/div/section/main/div/header/section/div[1]")
    ...
    def Login(self):
        self.driver = webdriver.Chrome("/Users/malamapono/Desktop/chromedriver")
    ...
    def GetInfo(self):
        ...
        temp = self.driver.find_element_by_xpath(
            "//*[@id='react-root']/div/div/section/main/div/header/section/ul/li[2]/a/span").text
        self.followersCount = self.ConvertToNumber(temp)

        # Get number of following
        temp = self.driver.find_element_by_xpath(
            "//*[@id='react-root']/div/div/section/main/div/header/section/ul/li[3]/a/span").text
    ...
    def FollowFollowers(self):
        ...
        self.driver.find_element_by_xpath("//*[@id='react-root']/div/div/section/main/div/header/section/ul/li[2]/a/span").click()
        i, k = 1, 1
        while (k <= amount):
            self.Wait(1, 1.5)
    ...
    def UnfollowAll(self):
        ...
        self.driver.find_element_by_xpath("//*[@id='react-root']/div/div/section/main/div/header/section/ul/li[3]/a").click()
        i, k = 1, 1
        while (k <= amount):
            self.Wait(1, 1.5)
            currentUser = self.driver.find_element_by_xpath("/html/body/div[6]/div/div/div[3]/ul/div/li[{}]".format(i))
            button = currentUser.find_elements_by_xpath(".//button")
            name = currentUser.find_element_by_css_selector(".notranslate").text
    ...
    def TagOptions(self):
        ...
        self.driver.find_element_by_xpath(
            '//*[@id="react-root"]/div/div/section/main/article/div[1]/div/div/div[1]/div[1]').click()

        for i in range(amount):
            self.Wait(40, 45)
            name = self.driver.find_element_by_xpath('/html/body/div[6]/div[2]/div/article/div/div[2]/div/div/div[1]/div/header/div[2]/div[1]/div[1]/span/a').text
            # Liking the post
            if liketag:
                self.Wait(1, 2)
            ...
            if commenttag:
                ...
                # post button
                self.driver.find_element_by_xpath('/html/body/div[6]/div[2]/div/article/div/div[2]/div/div/div[2]/section[3]/div/form/button[2]').click()
                print("[{}] *commented* on post by {}".format(i + 1, name))
            # Following the poster
            if followtag:
                self.Wait(1, 2)
                temp = self.driver.find_element_by_xpath(
                    '/html/body/div[6]/div[2]/div/article/div/div[2]/div/div/div[1]/div/header/div[2]/div[1]/div[2]/button')
            ...
            # Click to next post
            if i == 0:
                self.driver.find_element_by_xpath('/html/body/div[6]/div[1]/div/div/div/button').click()
            else:
                self.driver.find_element_by_xpath('/html/body/div[6]/div[1]/div/div/div[2]/button').click()


TestRuns = InstagramBot("kid_lenz", "Haaheo001")