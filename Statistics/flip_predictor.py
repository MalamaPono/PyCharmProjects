...
class FlipPredictor(object):
    ...
    def pheads(self):
        ...
        phead = 0
        for prob, head in zip(self.probs, self.coins):
            phead += prob * head
        return phead
    ...
    def update(self, result):
        ...
        pheads = self.pheads()
        if result == 'H':
            self.probs = [(prob * head / pheads) for prob, head in zip(self.probs, self.coins)]
        else:
            self.probs = [(prob * (1 - head) / (1 - pheads)) for prob, head in zip(self.probs, self.coins)]
